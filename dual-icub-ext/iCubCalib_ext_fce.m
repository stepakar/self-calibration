...
% rewritten as a function of DH parameters of estimated robot
function estimation_error = iCubCalib_ext_fce()
clc; %clear;
...
INIT_SETTINGS_DH = false;%if true, false loads computed parameters from a file estimated_parameters.mat
LOAD_DH_PAR = false;%if true, loads computed parameters from a file estimated_parameters.mat
ONLY_OFFSETS = true; %if true, loads computed offsets from a file estimated_parameters_offs.mat
...
%  These will be common to both models: real and estimate
   joints_torso_deg = [0.0   0.0   0.0]; % pitch, roll, yaw order (iKin, CAD, DH) - unlike iCub motor interfaces
   ...
   joints_Larm_deg  = [-23.6988   24.5989   42.5855   66.4666    0.0391         0   -3.5190];
